...
def convert_png_to_pdf(png_path):
    try:
        ...
        pdf_file_name = f"{base_name} (png converted into pdf).pdf"
        ...
        while os.path.exists(pdf_file_path):
            pdf_file_name = f"{base_name} ({counter}) (png converted into pdf).pdf"
            pdf_file_path = os.path.join(files_folder, pdf_file_name)
            counter += 1

        # Open the PNG image and convert it to PDF
...
def convert():
    ...
    if pdf_path:
        update_conversion_counter('PNG TO PDF')
...
# GUI setup
root = tk.Tk()
root.title("PNG to PDF Converter")
