...
def convert_gif_to_png(gif_path):
    try:
        ...
        png_file_name = f"{base_name} (converted to png).png"
        ...
        while os.path.exists(png_file_path):
            png_file_name = f"{base_name} ({counter}) (converted to png).png"
            png_file_path = os.path.join(files_folder, png_file_name)
            counter += 1

        # Open the GIF image and save it as PNG
...
# Select GIF file using file dialog
def select_file():
    file_path = filedialog.askopenfilename(filetypes=[("GIF Images", "*.gif")], title="Select a GIF file")
    if file_path:
...
# Convert the GIF file when button is clicked
def convert():
    gif_path = entry_file_path.get()
    ...
    png_path = convert_gif_to_png(gif_path)
    if png_path:
        update_conversion_counter('GIF TO PNG')
...
# GUI setup
root = tk.Tk()
root.title("GIF to PNG Converter")
