...
def convert_jpg_to_pdf(jpg_path):
    try:
        ...
        # Define initial PDF file path
        pdf_file_name = f"{base_name} (jpg converted into pdf).pdf"
        pdf_file_path = os.path.join(files_folder, pdf_file_name)

        # Check if the file exists, and if so, increment the name
        counter = 1
        while os.path.exists(pdf_file_path):
            pdf_file_name = f"{base_name} ({counter}) (jpg converted into pdf).pdf"
            pdf_file_path = os.path.join(files_folder, pdf_file_name)
            counter += 1

        # Open the JPG image and convert it to PDF
...
def convert():
    ...
    pdf_path = convert_jpg_to_pdf(jpg_path)
    if pdf_path:
        messagebox.showinfo("Success", f"File converted successfully to PDF:\n{pdf_path}")
        update_conversion_counter('JPG TO PDF')  
    else:
